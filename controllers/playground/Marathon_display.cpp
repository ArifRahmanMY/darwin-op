    ...
    Robot()
{
  ...
  //wd = mDisplay->getWidth();
  
  //mDisplay->setColor(0xFFFFFF);
  ...
  mVisionManager = new DARwInOPVisionManager(mCamera->getWidth(), mCamera->getHeight(), 360, 20, 50, 45, 0, 30);
  //Display mDisplay = Robot::getDisplay("display");
  // initialize display (speedometer)
  //display = wb_robot_get_device("display");
//  speedometer_image = wb_display_image_load(display, "speedometer.png");
  //wb_display_set_color(display, 0xffffff);
  //wb_display_draw_line(display, 10,10,10,10; 
}
...
void Marathon::run() {
  //wb_display_set_color(display, 0xFFFFFF); 
  //Robot::getDisplay("displaya")->setColor(0xFFFFFF);
  //Robot::getDisplay("display")->drawRectangle(10,10,10,10);
  //wb_display_draw_rectangle("display", 10,10,10,10);//int x, int y, int width, int height);
  ...
  while (true) {
    ...
    // go in the direction of the ball and kick it
      ...
      if (y < 0.1) // ball far away, go quickly
        ...
        mGaitManager->setXAmplitude(1.0);//0.5);
